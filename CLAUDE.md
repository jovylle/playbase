...
- **Deploy**: Cloudflare Workers + Static Assets, **manual only** — run `npm run deploy` (= `wrangler deploy`) locally after pushing. There is no CI auto-deploy; `.github/workflows/deploy.yml` was removed because it depended on a `CLOUDFLARE_API_TOKEN` repo secret that was never set, so every push silently failed to deploy while `master` looked green. Always deploy manually after merging changes that should go live.
